...
- **Sync foundation** - Vocabulary now carries sync metadata and a config-gated sync status UI for future Google Sheet syncing

## Recent Changes (April 11, 2026)

- Added a local-first sync foundation for vocabulary cards, including migration-safe metadata backfill for existing stores
- Added a sync status surface in Vocabulary that stays disabled until a sync backend is configured
- Refreshed the app icon and brand mark with a softer Japanese-inspired `J` seal
...
# Or open the committed Jouzu.xcodeproj in Xcode and hit Run
```
...
Google Sheet sync is not live by default in this repository. The app ships with the sync foundation and disabled sync UI, but it only attempts network sync when both of these Info.plist values are set:

- `GOOGLE_SHEET_SYNC_BASE_URL`
- `GOOGLE_SHEET_SYNC_API_KEY`

Keep real sync credentials out of the repo. Use local build settings or an uncommitted configuration override when you wire up a real backend.
